...
WebApp.connectHandlers.use(function(req, res, next) {
  ...
  dataHandle(req, res, async function(data) {
	...
	const runServerMethod = async (self) => {
      ...
      if (typeof methodCall === 'function') {
        ...
        try {
          ...
          thisScope.userId = await self.methodObject.auth.apply(self);
        } catch(err) {
        ...
        try {
          if (self.method === 'OPTIONS') {
            result = await methodCall.apply(thisScope, [self.methodObject]) || '';
          } else {
            result = await methodCall.apply(thisScope, [self.data]) || '';
          }
        } catch(err) {
          if (err instanceof Meteor.Error) {
            // Return controlled error
    ...
    try {
      await runServerMethod(fiberScope);
    } catch(err) {
